<?php
/**
 * This is a modified version of Jetpack's Nova Menu.
 *
 * @see jetpack/modules/custom-post-types/nova.php for the original
 *
...
class Pixassist_Nova_Menu {
	...
	public function register_hooks() {
		add_action( 'init', array( $this, 'add_shortcodes' ), 10 );
	...
	function site_supports_nova() {
		...
		if ( class_exists( 'Nova_Restaurant' ) && current_theme_supports( self::MENU_ITEM_POST_TYPE ) )
			return true;
		...
		if ( $query->have_posts() ) { ?>
			...
			<div class="jetpack-nova-menu-shortcode menu-list menu-list__<?php echo esc_attr( $atts['style'] ); ?>">
				...
				while ( $query->have_posts() ) {
					...
					$highlight = apply_filters( 'pixelgrade_nova_menu_shortcode_do_highlight', $highlight, $atts );

                    $has_highlight_title = false;
                    if ( true === $highlight && ! empty( $highlight_title ) ) {
